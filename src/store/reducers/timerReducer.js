...
const userReducer = (state = initialValues, { type, payload } = {}) => {
  switch (type) {
    ...
    case ADD_TIME_PRESSED_BUTTON: {
      const newTimesOfPressedButtons = [
        ...state.timesOfPressedButtons,
        payload,
      ];
    ...
    case ADD_CURRENT_LOG: {
      ...
      const timeOfPressedButtonInSec =
        timeOfPressedButton?.split(":")[0] * 3600 +
        parseInt(timeOfPressedButton?.split(":")[1] * 60) +
        parseInt(timeOfPressedButton?.split(":")[2]);
      const logShowTimeInSec =
        logShowTime.split(":")[0] * 3600 +
        parseInt(logShowTime.split(":")[1] * 60) +
        parseInt(logShowTime.split(":")[2]);
      const difference = logShowTimeInSec - timeOfPressedButtonInSec;
      const logObj = {
        button: payload.buttonNumber,
        timeOfPressedButton: timeOfPressedButton,
        endTime: logShowTime,
      ...
      return {
        ...state,
        logs: newLogs,
        timesOfPressedButtons: newTimesOfPressedButtons,
      };
    }
